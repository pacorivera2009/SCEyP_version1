...
{
    public partial class Rep_Totalapagar : Form
    {
        public Rep_Totalapagar()
        {
            InitializeComponent();
        }

        private void Rep_Totalapagar_Load(object sender, EventArgs e)
        {
            try
            {
                var bd = new Conexion();
                ...
                //IQueryable datos = bd.v_rep_totalespagos.Where(p => p.pag_fechapago <= fecha && p.C_p_pag_total___p_pag_pagado_ > 0).OrderBy(p => p.pag_fechapago);
